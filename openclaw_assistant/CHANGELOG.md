...
### Changed
- Merge upstream `techartdev/OpenClawHomeAssistant` changes through `0.5.63` into the WeLizard fork while keeping WeLizard source-mode defaults and the `Kiosk Scene` split.
- Show the active OpenClaw runtime mode directly on the add-on landing page, including the tracked Git repo/branch/commit in `source` mode and the bundled package version in `package` mode.
- Add a clear `package mode` warning so Home Assistant users can immediately see when WeLizard GitHub updates will not be pulled into the running add-on.
...
### Changed
- Release bump so Home Assistant detects the latest WeLizard OpenClaw update as a real add-on upgrade.
- In `install_mode: source`, this update pulls the latest `WeLizard/openclaw` `main` with wizard flow matching fixes, improved channel schema fallbacks, and better provider-auth behavior in Control UI.
...
### Changed
- Release bump to force Home Assistant update detection for the WeLizard add-on channel.

### Notes
- In `install_mode: source`, OpenClaw core updates are pulled from `https://github.com/WeLizard/openclaw.git` on add-on start.
- If the add-on is already installed, restart it once to fetch the latest `openclaw` `main` commit.
